...
**Not yet built / next (Mission Creator):** **T-057 map perf hotfix (urgent)** — pan/zoom must hold **60 fps with 200+ slots** (engineering contract; observed ~9 fps @ ~200). Then **Eden-first** — complete [`eden/gap_analysis.md`](Design_Docs/Mission_Creator_Architecture/eden/gap_analysis.md) **P0 remaining + P1 + P2** before Track A Phase 2 (map tiles, DEM). See [MC ROADMAP §Current strategy](Design_Docs/Mission_Creator_Architecture/ROADMAP.md#current-strategy-locked--2026-06). Eden next after perf: T-058+ (P1-07 faction submode, P1-05 multi-place, …). **Scale program (50k–100k editable entities)** is a multi-phase track after T-057 — see MC ROADMAP §Map performance.
